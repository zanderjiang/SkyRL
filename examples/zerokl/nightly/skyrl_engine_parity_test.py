...
PARITY_TP=2 runs the engine at TP=2 (matched-TP bring-up). vLLM then SPAWNS worker subprocesses that
re-import this file, so all executable code lives under the __main__ guard.
"""
import os
...
# PARITY_TP=2: matched-TP bring-up. The wrapper builds a TP-sharded Megatron GPTModel over vLLM's
# worker group; the zerokl vllm plugin (pyproject entry point) installs the CUSTOM backend + patches
# in each spawned worker.
PARITY_TP = int(os.environ.get("PARITY_TP", "1"))
...
def main():
    ...
    import skyrl.backends.skyrl_train.zerokl.varlen_backend as varlen_backend
    ...
    print(f"=== SkyRL zero-KL engine parity | torch {torch.__version__} | vllm {__import__('vllm').__version__} "
          f"| BI={vllm_envs.VLLM_BATCH_INVARIANT} N={N} TP={PARITY_TP} ===", flush=True)
    ...
    llm = LLM(
        ...
        dtype="bfloat16",
        enforce_eager=True,
        tensor_parallel_size=PARITY_TP,
    ...
    # (A) coherent generation (skipped when layer-tracing so trace forward indices stay clean)
    if not os.environ.get("SKYRL_ZEROKL_LAYERTRACE"):
    ...
    # PARITY_RESCORE_TWICE=1: rescore the SAME sequence a second time -- with layer tracing on, the
    # two rescore traces isolate nondeterminism (race) from decode-vs-prefill M-dependence.
    if os.environ.get("PARITY_RESCORE_TWICE") == "1":
        out3 = llm.generate([{"prompt_token_ids": full}],
                            SamplingParams(temperature=0.0, max_tokens=1, prompt_logprobs=0))[0]
        pl3 = out3.prompt_logprobs
        rep = [pl3[P + i][full[P + i]].logprob for i in range(n)]
        dd = [abs(rep[i] - prefill_lps[i]) for i in range(n)]
        print(f"[RESCORE-TWICE] second rescore vs first: max={max(dd):.6e} "
              f"exact0={sum(1 for d in dd if d == 0.0)}/{n}", flush=True)
    ...
    # PARITY_DUMP=<path>: save the rollout (tokens + engine decode/prefill logprobs) so a separate
    # torchrun TP-matched TRAINER scorer (trainer_score_tp.py) can score the SAME sequence and localize
    # any trainer-vs-engine residual without re-running the engine.
    _dump = os.environ.get("PARITY_DUMP")
    if _dump:
        torch.save({"prompt_ids": list(p2), "gen_ids": gen_ids,
                    "decode_lps": decode_lps, "prefill_lps": prefill_lps}, _dump)
        print(f"[DUMP] rollout + engine logprobs -> {_dump}", flush=True)
