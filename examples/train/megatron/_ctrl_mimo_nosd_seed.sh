...
# CONTROL EXPERIMENT (throwaway): faithful copy of run_megatron_dapo_mimo_7b_rl.sh (the NON-spec-decode
# baseline), parameterized by seed, with console logging + eval disabled so we can read per-step
# policy_entropy from stdout cheaply. Goal: is the baseline's entropy trajectory seed-sensitive on its
# own? If different seeds spread (some dipping toward the spec run's ~0.14), the sd-vs-nosd entropy gap
# is variance, not a bug. Everything except seed/logger/eval is identical to the baseline (TP=4 etc.).
#
# Usage: bash examples/train/megatron/_ctrl_mimo_nosd_seed.sh <SEED>
...
LOGGER="console"  # parse policy_entropy from stdout
...
uv run --isolated --extra megatron -m examples.train.algorithms.dapo.main_dapo \
  ...
  trainer.run_name="ctrl_nosd_seed${SEED}" \
  trainer.resume_mode=none \
  trainer.ckpt_path="/mnt/local_storage/ckpts/ctrl_nosd_seed${SEED}" \
  trainer.ckpt_interval=-1
