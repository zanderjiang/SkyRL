...
#
# This is the spec-decode counterpart of run_megatron_dapo_qwen3.5_4b_async.sh. Every knob
# below is IDENTICAL to the no-spec async script (same batch sizes, LR, parallelism, sampling,
# placement, staleness) so a reward-curve / throughput comparison is apples-to-apples. The ONLY
# difference is the `trainer.mtp.*` block at the bottom.
#
# Placement (colocate_all=false -- trainer and rollout on SEPARATE GPUs, run concurrently):
...
#   - Training side: builds + trains Qwen3.5-4B's native MTP head (the model ships
...
# Then launch:
#   bash examples/train/megatron/run_megatron_dapo_qwen3.5_4b_async_specdecode.sh
...
# Qwen3.5-4B ships 1 native MTP head (`mtp_num_hidden_layers: 1`); training always trains the
...
RUN_NAME="sd_async_dapo_qwen3_5_4b_megatron_tp${MEGATRON_TP}_pp${MEGATRON_PP}_cp${MEGATRON_CP}_${NUM_POLICY_GPUS}train${NUM_INFERENCE_ENGINES}gen_maxStale${MAX_STALENESS_STEPS}"
