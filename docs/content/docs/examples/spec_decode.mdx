...
2. **Drafting with it** — weight sync reloads the MTP head and its embedding / LM head through vLLM's native draft update session after syncing the policy.
...
<Callout type="warn">
MTP weight sync requires the default `nccl` backend. Delta, sharded RDT, serialized FP8, and adapter-only LoRA (`merge_lora=false`) are unsupported.
</Callout>
