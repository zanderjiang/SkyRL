...
- `remove_microbatch_padding`: Whether to pack samples and strip intra-microbatch padding during the model forward pass (requires flash attention; common for all models).
...
We support two backends: FSDP (PyTorch's composable `fully_shard` API, formerly known as FSDP2) and Megatron. The backend can be chosen with the `trainer.strategy` field.
...
- `cpu_offload`: Whether to train with CPU offloading (i.e., offload state during forward pass). This corresponds to [offload_policy](https://docs.pytorch.org/docs/stable/distributed.fsdp.fully_shard.html#torch.distributed.fsdp.fully_shard) in FSDP.
- `reshard_after_forward`: Whether to re-shard the FSDP model after the forward pass. See the [FSDP docs](https://docs.pytorch.org/docs/stable/distributed.fsdp.fully_shard.html#torch.distributed.fsdp.fully_shard) for details. If set to `false`, this would retain the full model parameters on each worker (similar to DeepSpeed's ZeRO stage 2).
- `fsdp_size`: The group size within which worker state is sharded with FSDP. This is a parameter to be used for hybrid sharding in multi-node settings. For example, if the number of workers in the actor group is 8, with 4 in each node, and `fsdp_size` is 4, then the training state will be fully sharded across 4 ranks in each node, but replicated (DP) across nodes.
    ...
    With FSDP, `cpu_offload` will offload parameter and optimizer state to CPU memory and only copy over model parameters to GPU during model forward pass.
...
policy:
  ...
  torch_profiler_config:    # torch.profiler for policy training
    enable: false
    ranks: [0]
...
`policy.torch_profiler_config` profiles policy training steps for FSDP and Megatron in RL and SFT. It writes one Kineto/[HTA](https://github.com/facebookresearch/HolisticTraceAnalysis)-friendly `*.pt.trace.json` per active window and profiled rank.

Scope: policy workers only. Active windows capture policy-worker operations between profiler steps: in RL, this includes policy log-prob forwards and policy training; in SFT, this includes training and may include eval forwards. Critic/ref workers, the controller, and generation/inference engines are not profiled.

FSDP restriction: profiling is rejected when colocation is enabled without CPU offloading (`policy.fsdp_config.cpu_offload: false` with `placement.colocate_all` or `placement.colocate_policy_ref`).

- `policy.torch_profiler_config.enable`: Enables profiling.
- `policy.torch_profiler_config.ranks`: Global ranks to profile, for example `[0]`.
- `policy.torch_profiler_config.save_path`: Required absolute local trace directory. Relative paths land under Ray's `/tmp/ray/.../working_dir_files`; cloud URIs are rejected.
- `policy.torch_profiler_config.{skip_first,wait,warmup,active,repeat}`: Passed to [`torch.profiler.schedule`](https://docs.pytorch.org/docs/stable/profiler.html#torch.profiler.schedule).
- `policy.torch_profiler_config.activities`: Subset of `["cpu", "cuda"]`.
- `policy.torch_profiler_config.{record_shapes,profile_memory,with_stack,with_flops,with_modules}`: Passed to `torch.profiler.profile`.
- `policy.torch_profiler_config.export_type`: `chrome_trace` for `*.pt.trace.json`; `stacks` for self-CUDA-time stacks (`with_stack: true` required).
...
algorithm:
  ...
  policy_loss_type: "regular" # "regular", "dual_clip", "gspo", "clip_cov", "kl_cov", "cispo", "sapo", "rollout_is", "dppo", or customizable with PolicyLossRegistry
...
- `algorithm.policy_loss_type`: Type of policy loss to use. Options include:
  ...
  - `sapo`: Soft Adaptive Policy Optimization ([SAPO](https://arxiv.org/html/2511.20347v1)) proposed by the Qwen team 
  - `rollout_is`: The agentic loss from section 4.1.2 of the [GLM-5 tech report](https://arxiv.org/pdf/2602.15763). Uses rollout logprobs and Icepop style clipping with additional stop gradient for masked tokens.
  - `dppo`: DPPO algorithm from [Rethinking the Trust Region in LLM Reinforcement Learning](https://arxiv.org/pdf/2602.04879). Uses rollout logprobs and absolute probability divergences rather than probability ratios to improve over PPO clipping behavior.
...
- `generator.inference_engine.run_engines_locally`: Whether to launch inference servers during the training run in the current Ray cluster. If `false`, point SkyRL at an external HTTP/vLLM deployment with `external_proxy_url` and/or `external_server_urls`.
- `generator.inference_engine.num_engines`: Number of inference engines to launch when `run_engines_locally=true`.
- `generator.inference_engine.external_proxy_url`: Optional data-plane URL for an external router/proxy. Generation requests are sent here.
- `generator.inference_engine.external_server_urls`: Optional list of external backend server URLs for control-plane fan-out (pause/resume, sleep/wake, and weight sync). If `external_proxy_url` is omitted, SkyRL starts an internal router over these servers.
...
- `generator.inference_engine.weight_sync_backend`: Backend to use for weight synchronization. Use `nccl`; colocated `nccl` uses CUDA IPC internally.
...
- Remote inference servers automatically replace any stale weight update group during initialization. Locally launched inference servers do not.
...
- `generator.inference_engine.pipeline_parallel_size`: Pipeline parallel size for the inference engine. Currently, PP is only supported for the vLLM backend.
...
- `generator.inference_engine.engine_init_kwargs`: Pass-through kwargs for vLLM engine initialization. For Hugging Face config overrides such as RoPE scaling, use `generator.inference_engine.engine_init_kwargs.hf_overrides.rope_parameters` and set the same trainer-side override with `trainer.policy.model_config_kwargs.rope_parameters` or `trainer.policy.megatron_config.transformer_config_kwargs.rope_parameters`.
...
- `generator.inference_engine.enable_prefix_caching`: Whether to enable prefix caching for the inference engine. Applicable only when `backend="vllm"`. This can be left to the default `true` in most cases. Note that in the case of remote inference servers, you would need to match the setting used when you initialized the remote servers.
...
- `trainer.use_expandable_segments` (default **on**) — most directly mitigates colocated backload fragmentation, but also helps fwd/bwd activation fragmentation in non-colocated runs. Under `colocate_all=false`, weight sync uses NCCL broadcast rather than CUDA-IPC, so it simply stays on continuously.
- `generator.inference_engine.use_expandable_segments` (default **off**) — enable it to also defragment the engine side; on vLLM `< 0.20.1`, sleep mode plus expandable segments is a hard error, so leave it off there.
