...
def test_pooled_session_cleanup_runs_on_its_owner_loop(owner_state):
    ...
    try:
        ...
        if owner_state == "closed":
            ...
            assert not old.closed  # Other live loops keep their own pool.
            asyncio.run_coroutine_threadsafe(client.aclose(), owner).result(timeout=5)
        else:
